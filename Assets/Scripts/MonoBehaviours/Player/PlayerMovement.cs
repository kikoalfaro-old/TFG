...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Debug.Log("Starting position: " + startingPosition.gameObject == null);
    ...
    private void Stopping (out float speed)
    {
        ...
        if (currentInteractable)
        {
            ...
            // --- > Esto se tendría que llamar desde el FUNGUS. Las interacciones se configuran en el inspector, pero se manejarán desde allí.
            //currentInteractable.Interact();
            Flowchart.BroadcastFungusMessage(currentInteractable.name);
    ...
    private void Moving ()
    {
        ...
        // Lo ideal sería flippear el sprite (si la x pulsada está a la derecha o la izquierda de la posición actual...) o algo más simple
        if (targetRotation.y > 0) transform.rotation = Quaternion.Euler(new Vector3(0, 0, 0));
    ...
    public void OnGroundClick(BaseEventData data)
    {
        ...
        // Pongo la rotación de momento. Después con el DoTween lo animaré o lo que sea...
        if(destinationPosition.x >= transform.position.x) transform.rotation = Quaternion.Euler(new Vector3(0, 0, 0));
        else transform.rotation = Quaternion.Euler(new Vector3(0, 180, 0));
    }
